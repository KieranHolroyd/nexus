...
func main() {
	if len(os.Args) < 2 {
		fmt.Println("Usage:")
		fmt.Println("  go run scripts/dump_nexus.go <url_or_db_path>")
		fmt.Println("\nExamples:")
...
func fetchFromAPI(baseURL string) []Service {
	...
	filtered := []Service{}
	for _, m := range all {
		// Filter for public services that don't require auth (what's on the public page)
		pub, _ := m["public"].(bool)
...
func readFromDB(dbPath string) []Service {
	fmt.Fprintf(os.Stderr, "Reading services from database %s...\n", dbPath)
	...
	rows, err := db.Query(`SELECT name, url, icon, "group", "order", public, auth_required, new_tab FROM services WHERE public = 1 AND auth_required = 0 ORDER BY "order" ASC`)
	if err != nil {
