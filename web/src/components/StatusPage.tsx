...
import { UptimeView, type UptimeHistory } from "./UptimeView";
...
function LazyServiceCard({ 
  ...
  service: Service, 
  index: number, 
  bulkUptime: Record<string, UptimeHistory> | undefined 
}) {
    ...
    >
      <div className="flex items-center gap-4 mb-4 ml-1">
        <div className="size-12 rounded-2xl bg-muted/40 border-2 flex items-center justify-center p-2 group-hover:bg-primary/5 transition-colors">
          {service.icon ? (
            <img src={service.icon} alt="" className="size-full object-contain grayscale group-hover:grayscale-0 transition-all" />
          ) : (
          ...
          <div className="flex items-center gap-3">
            ...
            <Badge 
              ...
              className={cn(
                ...
                service.health_status === "online" ? "bg-green-500/5 text-green-600 border-green-500/20" : 
                service.health_status === "offline" ? "bg-red-500/5 text-red-600 border-red-500/20" : 
...
export function StatusPage({ search = "" }: StatusPageProps) {
  ...
  const filteredServices = monitoredServices.filter((s) => 
